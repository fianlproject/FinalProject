		...
		<div class="content">
					...
					<c:if test="${cpcheck==1}">
						...
						<form method="post" action="/pfinal/pfinal.do">
							<button class="btn btn-link pl" name="command"
								value="privatelist">비공개게시글</button>
						</form>
						...
						<form method="post" action="/pfinal/pfinal.do">
							<button class="btn btn-link pl" name="command"
								value="partnerlist">파트너목록</button>
						</form>
			...
			<div class="content-inner">
				<section class="project-unit"> <c:forEach
					items="${dtoList}" var="dto" begin="${startpage-1}"
					end="${endpage-1}">
					...
					<img src="img/empty.png"
						style="background-image: url('img/hr_line.png'); background-repeat: repeat-x; width: 100%; height: 30px; background-position: center; margin-bottom: 15px; margin-top: 10px" />

				</c:forEach> <!-- //사이즈 --> <c:forEach begin="1" end="${size}" step="1"
					varStatus="count1">
