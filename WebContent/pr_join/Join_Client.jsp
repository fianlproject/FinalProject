...
<body>
	<div class="navbar navbar-inverse">
		<div class="navbar-inner">
			<label class="brand">기업회원 가입하기</label>
		</div>
	</div>
	<form method="post" action="/pfinal/pfinal.do" class="form-horizontal">
	<input type="hidden" name="command" value="join_client" />
	<input type="hidden" name="cpcheck" value="1"/>
		...
		<div class="form-group">
			...
			<div class="controls">
				<input type="password" name="pwConfirm" id="pwConfirm" class="form-control" required="required"/>
			</div>
		...
		<div class="form-group">
			<label class="control-label">COMPANY</label>
				<div class="controls">
					<input type="text" name="cname" id="cname" class="form-control" required="required"/>
				</div>
		</div>
		<hr />
		<div class="form-group">
	...
	$(document).ready(function() {    
	    ...
	    var $form = $("#f1"), $id = $("[name=id]"), $pw = $("[name=pw]"), 
	    			$pw1 = $("[name=pwConfirm]"), $pname = $("[name=pname]");
	    ...
	    $form.submit( function() {
			...
			else if(re_pname.test($pname.val()) != true){
				alert('이름을 다시 입력해주세요.');
				$name.focus();
				return false;
			}
