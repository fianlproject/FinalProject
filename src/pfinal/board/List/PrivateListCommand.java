...
public class PrivateListCommand implements Command {
	...
	public Object processCommand(HttpServletRequest req, HttpServletResponse res)
		...
		if(KeyWord==null||KeyWord.isEmpty()){
			 sql = "select  pr_id,pr_subject,pr_skill,pr_content,pr_price,"
				...
				+ "Floor((to_days(pr_end)-to_days(now()))/7) as week_cha, (to_days(pr_end)-to_days(now()))%7 as day_cha from list where pr_private='n' order by pr_id desc";
		}else{
			sql ="select  pr_id,pr_subject,pr_skill,pr_content,pr_price,"
				+ "pr_start,pr_end,pr_needman,pr_cntman,c_name,pr_photo,cl_id,"
				+ "Floor((to_days(pr_end)-to_days(now()))/7) as week_cha, (to_days(pr_end)-to_days(now()))%7 as day_cha from list where pr_private='n' "+ KeyFiled
					+" like '%"+ KeyWord + "%' order by pr_id desc";
		}
		...
		try {
			...
			for(int k=0;k<vc.size();k++){
				...
				System.out.println(dto1.getPr_id123()+"$$$$$$$$");
				ck[k]=dto1.getCheck123();
				ck1[k]=dto1.getPr_id123();
				System.out.println(ck[k]+","+ck1[k]);
	         }
